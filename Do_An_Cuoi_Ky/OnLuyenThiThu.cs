    ...
    public partial class frmOnLuyenThiThu : Form
    {
        ...
        private void frmOnLuyenThiThu_Load(object sender, EventArgs e)
        {
            ...
            using (DatabaseDataContext db = new DatabaseDataContext())
            {
                ...
                //var dsThiSinh = db.KyThiThus.Where(ktt => ktt.DanhSachThiSinh.Contains(stt.ToString())).Select(ktt => ktt.DanhSachThiSinh).ToArray();
                ...
                //        tontai = true;
                bool tontai = true;
                var thiSinh = db.KyThiThus.Where(ktt => ktt.DanhSachThiSinh.Contains(stt.ToString()) && ktt.TuNgay <= DateTime.Now.Date && ktt.DenNgay >= DateTime.Now.Date);
                if (thiSinh.Count() == 0)
                    tontai = false;
                ...
                else
                {
                    if(NguoiDungHienTai.LayLoai().Trim() == "HocSinh")
                    {
                        ...
                        var made = db.KyThiThus.Where(tt => tt.DanhSachThiSinh.Contains(stt.ToString())).Where(tt => tt.TuNgay <= DateTime.Now.Date).Where(tt => tt.DenNgay >= DateTime.Now.Date).Select(tt => tt.DanhSachDeThi).ToArray();
                    ...
                    {
                        var made = db.DeThis.Select(dt => dt.MaDe).ToArray();
                        cbbChonDe.DataSource = made;
                        LoadDe(cauHoi);
                    }
                } 
            }
