...
namespace Do_An_Cuoi_Ky
{
    public partial class frmQuanLyDeThi : Form
    {
        ...
        private void btnTaoDe_Click(object sender, EventArgs e)
        {
            ...
            using (DatabaseDataContext db = new DatabaseDataContext())
            {
                if(db.DeThis.Where(dt => dt.MonHoc.Equals(cbbTheoMonHoc.Text)).ToArray().Count() <= 0)
                {
                    //Trường hợp môn đó chưa có đề thi.
                    string made = "";
                    if (cbbTheoMonHoc.SelectedValue.ToString().Trim() == "Toán")
                        made = "TT";
                    else if (cbbTheoMonHoc.SelectedValue.ToString().Trim() == "Hóa Học")
                        made = "HH";
                    else if (cbbTheoMonHoc.SelectedValue.ToString().Trim() == "Vật Lý")
                        made = "VL";
                    else if (cbbTheoMonHoc.SelectedValue.ToString().Trim() == "Lịch Sử")
                        made = "LS";
                    string maDeMoi = made + "01";
                ...
                {
                    //Trường hợp môn đó đã có ít nhất 1 đề trong csdl đề.
